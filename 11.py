...
#f.putpixel((0,0),(255,0,0))
#print f.getpixel((0,0))
#halfsize = tuple([n/2 for n in f.size])

halfsize =(800,800)
#odd = Image.new(f.mode,halfsize)
even = Image.new(f.mode,halfsize)


for x in xrange(f.size[0]):
    for y in xrange(f.size[1]):
        if x % 2 ==0 and y %2 == 0:
            even.putpixel((x/2,y/2),f.getpixel((x,y)))
            #f.putpixel((x,y),(255,0,0))
